...
OFFSET_FILES = {
    "NG_IDRefseq": os.path.join(PROC_DIR, "Output", "Step2_RefCheck", "lrg_offset_results.csv"),
    "NG_MANE":     os.path.join(PROC_DIR, "Output", "Step2_RefCheck", "MANE_offset_results.csv"),
}
...
def analyze(all_mut, unique_vars, offset_df, accession_col):
    rows = []
    for _, off in offset_df.iterrows():
        gene   = off["gene"]
        status = off["status"]
        strand = off.get("strand", "")
        acc    = off.get(accession_col, "")
        ...
        if pd.notna(off["sstart"]) and pd.notna(off["send"]):
            ...
            sub_rows = all_mut[
                ...
                (all_mut["mut_class"] == "substitution")
            ].dropna(subset=["pos_start"])

            sub_rows  = sub_rows.drop_duplicates(subset=["accession", "allele_num"])
            positions = sorted(sub_rows["pos_start"].astype(int).tolist())
...
def main():
    ...
    for track_name, offset_path in OFFSET_FILES.items():
        ...
        accession_col = "ng_accession"

        out_df   = analyze(all_mut, unique_vars, offset, accession_col)
        out_path = os.path.join(OUT_DIR, f"gene_confidence_{track_name}.csv")
        out_df.to_csv(out_path, index=False)
        print(f"[{track_name}] Wrote {len(out_df)} rows to {out_path}")

        ok = out_df[out_df["status"].isin(["ok", "below_threshold"])]
        ...
        print(f"  p<0.05: {(ok_p['binomial_p']<0.05).sum()}/{len(ok_p)}  "
              ...
              f"p>=0.05: {(ok_p['binomial_p']>=0.05).sum()}/{len(ok_p)}")
        print(f"  coverage_frac>1: {(ok['coverage_frac']>1).sum()} (should be 0 after fix)")
        print()
